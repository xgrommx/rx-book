# Video

* #### Expert to Expert: Brian Beckman and Erik Meijer - Inside the .NET Reactive Framework (Rx)
    {% video %}https://www.youtube.com/watch?v=looJcaeboBY{% endvideo %}
    ***
* #### Matthew Podwysocki - Streaming and event-based programming using FRP and RxJS (FutureJS 2014)
    {% video %}https://www.youtube.com/watch?v=zlERo_JMGCw{% endvideo %}
    ***
* #### Netflix JavaScript Talks - Falcor
    {% video %}https://www.youtube.com/watch?v=z8UgDZ4rXBU{% endvideo %}
    ***
* #### Netflix JavaScript Talks - Version 7: The Evolution of JavaScript
    {% video %}https://www.youtube.com/watch?v=DqMFX91ToLw{% endvideo %}
    ***
* #### Netflix JavaScript Talks - Async JavaScript with Reactive Extensions
    {% video %}https://www.youtube.com/watch?v=FAZJsxcykPs{% endvideo %}
    ***
* #### Keynote - JSON Graph: Reactive REST at Netflix
    {% video %}https://www.youtube.com/watch?v=hOE6nVVr14c{% endvideo %}
    ***
* #### Matthew Podwysocki: Async and Streaming JS - Are we doing it wrong? | JSConf EU 2014   
    {% video %}https://www.youtube.com/watch?v=-vPFP-2Mkl8{% endvideo %}
    ***
* #### Philly ETE 2014 #39 - Reactive Javascript at Netflix - Jafar Husain & Matthew Podwysocki
    {% video %}https://www.youtube.com/watch?v=CaNqyhxMNk8{% endvideo %}
    ***
* #### Reactive all the Things - Ben Lesh and Martin Gontovnikas
    {% video %}https://www.youtube.com/watch?v=zbBVG8bOoXk{% endvideo %}
    ***
* #### Ian Hansen - RxJS for Metrics and Music
    {% video %}https://www.youtube.com/watch?v=2btEt0W7UxU{% endvideo %}
    ***
* #### Getting the most out of $http and Intro to rxjs (and the angular plugin)
    {% video %}https://www.youtube.com/watch?v=3I99MjfRPbc{% endvideo %}
    ***
* #### MountainWest JavaScript 2014 - Adding Even More Fun to Functional Programming With RXJS
    {% video %}https://www.youtube.com/watch?v=8EExNfm0gt4{% endvideo %}
    ***
* #### RxJS
    {% video %}https://www.youtube.com/watch?v=GEyYGGdbA90{% endvideo %}
    ***
* #### CascadiaJS 2012 - Matthew Podwysocki
    {% video %}https://www.youtube.com/watch?v=FqBq4uoiG0M{% endvideo %}
    ***
* #### Netflix JavaScript Talks - Async JavaScript with Reactive Extensions
    {% video %}https://www.youtube.com/watch?v=XRYN2xt11Ek{% endvideo %}
    ***
* #### MountainWest JavaScript 2014 - Asynchronous JavaScript at Netflix by Jafar Husain
    {% video %}https://www.youtube.com/watch?v=XE692Clb5LU{% endvideo %}
    ***
* #### HTML5DevConf Jafar Husain, Netflix: Asyncronous JavaScript at Netflix
    {% video %}https://www.youtube.com/watch?v=5uxSu-F5Kj0{% endvideo %}
    ***
* ####  Bodil Stokke: Reactive Game Development For The Discerning Hipster [JSConf2014]
    {% video %}https://www.youtube.com/watch?v=x8mmAu7ZR9Y{% endvideo %}
    ***
* #### A Playful Introduction to Rx by Erik Meijer
    {% video %}https://www.youtube.com/watch?v=WKore-AkisY{% endvideo %}
    ***
* #### HTML5DevConf: Jafar Husain, Netflix: Version 7: The Evolution of the JavaScript
    {% video %}https://www.youtube.com/watch?v=Oru-qw-Faac{% endvideo %}
    ***
* #### Async Programming at Netflix by Matthew Podwysocki at Web Rebels 2015
    {% video %}https://vimeo.com/128858567{% endvideo %}
    ***
* #### Reactive Programming for Java Programmers
    {% video %}https://www.youtube.com/watch?v=fz31sbwOYq8{% endvideo %}
    ***
* #### Asynchronous JavaScript at Netflix by Matthew Podwysowski at JSConf Budapest 2015
    {% video %}https://www.youtube.com/watch?v=a8W5VVGO-jA{% endvideo %}
    ***
* #### What if the user was a function? by Andre Staltz at JSConf Budapest 2015
    {% video %}https://www.youtube.com/watch?v=1zj7M1LnJV4{% endvideo %}
    ***
* #### Async and Streaming JavaScript, We're All Doing it Wrong! - Matthew Podwysocki
    {% video %}https://vimeo.com/131196784{% endvideo %}
    ***
* #### Reactive Programming
    {% video %}https://www.youtube.com/watch?v=3bAQXTVsEiQ{% endvideo %}
    ***
* #### Intro to Functional Reactive Programming with Cycle.js
    {% video %}https://www.youtube.com/watch?v=6_ETUyh0tns{% endvideo %}
    ***
* #### React 2014 : Erik Meijer - What does it mean to be Reactive?
    {% video %}https://www.youtube.com/watch?v=sTSQlYX5DU0{% endvideo %}
    ***
* #### Evan Czaplicki - Let's be mainstream! User focused design in Elm - Curry On
    {% video %}https://www.youtube.com/watch?v=oYk8CKH7OhE{% endvideo %}
    ***
* #### Functional Reactive Programming in Elm
    {% video %}https://vimeo.com/105188956{% endvideo %}
    ***
* #### John P Mayer - Delightful WebGL in Elm
    {% video %}https://vimeo.com/97408205{% endvideo %}
    ***
* #### DevNation 2015 - Leathem - Transform your web applications with reactive functional programming
    {% video %}https://www.youtube.com/watch?v=dfy1pGlU8Jk{% endvideo %}
    ***
* #### Reactive Function with RxJS
    {% video %}https://www.youtube.com/watch?v=v2bknh5ridY{% endvideo %}
    ***
* #### Functional Reactive Programming in your browser by Vagmi Mudumbai
    {% video %}https://www.youtube.com/watch?v=LOJS4weW5u4{% endvideo %}
    ***
* #### Erik Hinton on Functional Reactive Programming (TimesOpen: Reactive Programming)
    {% video %}https://www.youtube.com/watch?v=ZD3QAi0FlyE{% endvideo %}
    ***
* #### Reactive Streams in Angular 1 and 2 Ben Lesh
    {% video %}https://www.youtube.com/watch?v=KOOT7BArVHQ{% endvideo %}
    ***
* #### "Controlling Time and Space: understanding the many formulations of FRP" by Evan Czaplicki
    {% video %}https://www.youtube.com/watch?v=Agu6jipKfYw{% endvideo %}
    ***
* #### RxJS used to build a game (enhanced game features)
    {% video %}https://www.youtube.com/watch?v=AJAKUqy5Uqc{% endvideo %}
    *** 
* #### The functional reactive frontend
    {% video %}https://www.youtube.com/watch?v=ptOmAIqNlFI{% endvideo %}
    ***
* #### .NET Conf UY v2015 - Building Reactive Services using Functional Programming by Rachel Reese
    {% video %}https://www.youtube.com/watch?v=l1tZ8She768{% endvideo %}
    ***
* #### Lambda Jam 2015 - Conal Elliott - The Essence and Origins of Functional Reactive Programming
    {% video %}https://www.youtube.com/watch?v=j3Q32brCUAI{% endvideo %}
    ***    
* #### Reactive Programming
    {% video %}https://www.youtube.com/watch?v=GHtARIwwc30{% endvideo %}
    ***      
* #### I have a stream – Einsichten in Reactive Programming
    {% video %}https://www.youtube.com/watch?v=qHKfwlQGI_o{% endvideo %}
    ***      
* #### Reactive.community: Ben Christensen, Reactive Extensions (Rx) at Netflix
    {% video %}https://www.youtube.com/watch?v=ET_SMMXkE5s{% endvideo %}
    ***      
* #### An Introduction to Functional Reactive Programming using [Sir Francis]Bacon.js
    {% video %}https://www.youtube.com/watch?v=7PK5h3BVGds{% endvideo %}
    ***         
* #### Real-Time Deadlines in Functional-Reactive Programming
    {% video %}https://www.youtube.com/watch?v=waJ4n1sGiRQ{% endvideo %}
    ***                     
* #### Akka.NET with F# Getting functional with Reactive systems - Russ Cam (@forloop)
    {% video %}https://www.youtube.com/watch?v=6G4-bToLMWY{% endvideo %}
    ***
* #### Bacon.js for Breakfast: An intro to functional reactive programming
    {% video %}https://www.youtube.com/watch?v=SNMVh73rU_4{% endvideo %}
    ***
* #### Angular 2 Data Flow
    {% video %}https://vimeo.com/144625829{% endvideo %}
    ***
* #### Angular 2 Data Flow – Jeff Cross, Rob Wormald and Alex Rickabaugh
    {% video %}https://www.youtube.com/watch?v=bVI5gGTEQ_U{% endvideo %}
    ***
* #### Philip Roberts: Enemy of the state: An introduction to Functional Reactive Programming and Bacon.js
    {% video %}https://vimeo.com/68987289{% endvideo %}
    ***
* #### Wellington FP - Why Functional Reactive Programming FRP?
    {% video %}https://www.youtube.com/watch?v=gaG3tIb3Lbk{% endvideo %} 
    ***
* #### Jeffrey Van Gogh: The Reactive Extensions for JavaScript
    {% video %}https://www.youtube.com/watch?v=ClHpkn_qxos{% endvideo %}
    ***
* #### Bacon.js - Functional Reactive Programming - Thomas Holmes - ATLRUG 2014
    {% video %}https://www.youtube.com/watch?v=eFEvGS6pOWk{% endvideo %}    
    ***
* #### Functional Reactive Programming with RxJava • Ben Christensen
    {% video %}https://www.youtube.com/watch?v=_t06LRX0DV0{% endvideo %}
    ***
* #### Juha Paananen. FRP with Bacon.js
    {% video %}https://www.youtube.com/watch?v=3HS3i8LW8dQ{% endvideo %}   
    ***
* #### "PureScript (Maybe This Time We Get JavaScript Right)" by Bodil Stokke
    {% video %}https://www.youtube.com/watch?v=yIlDBPiMb0o{% endvideo %}
    ***
* #### PolyConf 15: Post FRP Frontend Programming / Bodil Stokke
    {% video %}https://www.youtube.com/watch?v=X5YBsy6PaDw{% endvideo %}
    ***
* #### Remote Talk - Introduction to Functional Reactive Programming with Elm
    {% video %}https://www.youtube.com/watch?v=bx3mOTGRvs4{% endvideo %}
    ***    
* #### Netflix JavaScript Talks - RxJS Version 5
    {% video %}https://www.youtube.com/watch?v=COviCoUtwx4{% endvideo %}
    ***
* #### FluentConf 2016 - Building our reactive future - Matthew Podwysocki (Microsoft)
    {% video %}https://www.youtube.com/watch?v=IB4BHRtu2MA{% endvideo %}
    ***
* #### Elm - An Introduction - by Randall Britten at Movio
    {% video %}https://www.youtube.com/watch?v=y9N6JBKNKas{% endvideo %}
    ***
* ### A Reactive Typeahead: A Soft Intro to Observables and CycleJS
    {% video %}https://www.youtube.com/watch?v=M60845GVIDU{% endvideo %}
    ***
* ### Droidcon SF - Common RxJava Mistakes
    {% video %}https://www.youtube.com/watch?v=QdmkXL7XikQ{% endvideo %}
    ***
* ### Richard Feldman - Introduction to ELM (March 22, 2016)
    {% video %}https://www.youtube.com/watch?v=3_M2G9U51GA{% endvideo %}